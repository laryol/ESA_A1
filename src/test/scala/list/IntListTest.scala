...
class IntListTest extends AnyFunSuite {
  ...
  test("testReduceRight is right to left") {
    assert(SinglyLinkedIntList(3, 2, 1).reduceRight((x, y) => {
      if (x > y)
        y
      else fail()
    }) === 1)
  ...
  test("testFoldLeft NEW____________________________________________") {
    assert(SinglyLinkedIntList(1, 2, 3).foldLeft(5)((x, y) => x + y) === 11)
  }
  ...
  test("testFoldLeft generic is left to right") {
    assert(SinglyLinkedIntList(1, 2, 3).foldLeft(0.0f)((x, y) => {
      if (x > y)
        fail()
      else y
    }) === 3)
